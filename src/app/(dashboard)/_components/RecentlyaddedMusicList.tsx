...
function RecentlyaddedMusicList() {
  ...
  const getStatusBadge = (status: any) => {
    ...
    if (status === "Active") {
      return `${baseClasses} bg-green-900/30 border-green-700 text-green-400`;
    } else if (status === "Pending") {
      return `${baseClasses} bg-red-900/30 border-red-700 text-red-400`;
    }
  ...
  return (
    <div className="border border-gray-700 rounded-2xl p-6 w-full">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm">Latest uploads to your library</p>
      </div>
        ...
        {musicData.map((track) => (
          ...
          >
            ...
            <div className="flex-shrink-0">
              <span className={getStatusBadge(track.status)}>
                {track.status}
              </span>
            </div>
          </div>
